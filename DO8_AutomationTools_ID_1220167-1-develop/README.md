...
Проект посвящен проектированию, развертыванию и комплексной автоматизации распределенной гетерогенной серверной инфраструктуры на базе инструмента управления конфигурациями **Ansible** и реализации отказоустойчивой системы обнаружения сервисов (**Service Discovery**) с динамическим управлением сетевой связностью (**Service Mesh**) на базе **HashiCorp Consul** и **Envoy Proxy**. В рамках работы спроектированы и практически реализованы два многоузловых изолированных стенда виртуализации на базе **HashiCorp Vagrant** и гипервизора **Oracle VirtualBox** под управлением **Ubuntu 24.04 LTS**.

В первом сценарии решена задача безагентного декларативного провижининга целевых серверов по протоколу SSH, декомпозиции монолитных сценариев на модульные идемпотентные роли Ansible (`application`, `apache`, `postgres`), автоматической установки Docker Engine, оркестрации девятикомпонентного микросервисного стека через Docker Compose, а также развертывания веб-сервера Apache HTTP Server и СУБД PostgreSQL с настройкой сетевого доступа. Во втором сценарии развернут распределенный кластер HashiCorp Consul (Server + Clients), использующий алгоритм консенсуса Raft и протокол Serf LAN Gossip. Поверх кластера настроен Service Mesh с sidecar-прокси Envoy, mTLS-шифрованием трафика, регистрацией сервиса бронирования отелей (Spring Boot Java 21) и СУБД PostgreSQL, проверками состояния узлов (Health Checks) и подтвержденной устойчивостью к динамической миграции IP-адресов серверов в режиме Zero-Downtime. Корректность функционирования всей инфраструктуры подтверждена сквозным тестированием через Postman и Newman.

---

## О проекте

Современные распределенные программные комплексы предъявляют жесткие требования к воспроизводимости окружений, скорости развертывания и надежности межсервисного сетевого взаимодействия. Данный проект направлен на устранение фундаментальных проблем системного администрирования и эксплуатации:

### 1. Проблематика ручной настройки и «дрейфа конфигураций» (Configuration Drift)
При ручном конфигурировании серверов через терминал неизбежно накапливаются расхождения между тестовыми, препродакшен и продуктовыми контурами: различия в версиях утилит, неучтенные переменные окружения, несинхронизированные параметры сетевых экранов и различия в системных библиотеках. Подобный «дрейф конфигураций» приводит к ошибкам, которые невозможно воспроизвести в изолированной среде, увеличивает время восстановления после сбоев (MTTR) и превращает масштабирование инфраструктуры в трудоемкий процесс.

### 2. Принципы Infrastructure as Code (IaC) и идемпотентность в Ansible
Парадигма «Инфраструктура как код» переводит управление вычислительными ресурсами в плоскость версионируемых текстовых манифестов. В основе Ansible лежит концепция **декларативности** и **идемпотентности**: администратор описывает желаемое конечное состояние системы (какие пакеты должны присутствовать, какие службы запущены, какие права назначены на файлы), а модуль Ansible самостоятельно определяет необходимые корректирующие действия. Повторное применение плейбука к уже настроенной системе завершается статусом `ok=... changed=0`, гарантируя отсутствие нежелательных побочных эффектов.

### 3. Безагентная архитектура (Agentless via SSH)
В отличие от Chef, Puppet или SaltStack, требующих установки, обновления и мониторинга специализированных фоновых демонов-агентов на каждом управляемом узле, Ansible использует стандартную подсистему OpenSSH и системный интерпретатор Python. Это исключает накладные расходы на оперативную память, устраняет необходимость открытия дополнительных служебных сетевых портов и минимизирует потенциальную поверхность атаки на хосты.

### 4. Необходимость Service Discovery в микросервисных архитектурах
В монолитных системах межмодульные вызовы производятся локально в оперативной памяти, а связь с базой данных настраивается через статический IP-адрес или DNS-запись. В микросервисных средах с десятками независимых контейнеров, динамическим автомасштабированием, перезапусками при сбоях и миграцией между гипервизорами использование статически зафиксированных IP-адресов приводит к мгновенной деградации системы при любом изменении топологии. **Service Discovery** решает эту проблему за счет ведения централизованного динамического реестра сервисов (Service Registry), автоматической регистрации экземпляров при старте, непрерывного контроля жизнеспособности (Health Checking) и предоставления актуальных эндпоинтов через DNS или HTTP API.
...
- **Serf Gossip (протокол SWIM):** децентрализованный обмен сообщениями по протоколу UDP/TCP (порт `8301`). Каждый узел периодически зондирует случайных соседей, что обеспечивает быстрое (субсекундное) обнаружение отказов узлов без создания квадратичной нагрузки на центральные серверы.
- **Алгоритм консенсуса Raft:** обеспечивает строгую согласованность (strong consistency) для сервис-каталога и хранилища Key-Value (порт `8300`). Серверный кластер производит автоматические выборы лидера (Leader Election) и распределяет журнал репликации между узлами-участниками.

### 6. Концепция Service Mesh на базе Envoy Proxy
Включение технологии **Consul Connect** позволяет трансформировать Consul в Control Plane для распределенного Service Mesh. Роль Data Plane берет на себя **Envoy Proxy**, запускаемый как sidecar-процесс рядом с каждым сервисом. Приложения взаимодействуют исключительно с локальным интерфейсом loopback (`127.0.0.1`), а Envoy перехватывает трафик, запрашивает актуальные маршруты у локального Consul Client через постоянный gRPC-канал (порт `8502`, протокол xDS) и передает данные целевому сервису через защищенный взаимно аутентифицированный туннель (mTLS). Физическое перемещение базы данных на другой IP-адрес становится прозрачным для клиентского микросервиса: маршрут обновляется на лету без изменения исходного кода и без перезапуска приложений.
...
- **Декларативное управление инфраструктурой:** проектирование инвентаря `inventory.ini` с логическим разделением хостов на группы (`[managers]`, `[nodes]`), фиксацией системного интерпретатора `ansible_python_interpreter=/usr/bin/python3` для устранения недетерминированного автоопределения и предупреждений Ansible.
- **Организация безагентного транспорта:** генерация криптографических пар SSH-ключей на управляющем узле `manager`, автоматизированный экспорт публичных ключей через `ssh-copy-id` на целевые узлы `node01` и `node02`, валидация сетевой доступности модулем `ansible.builtin.ping`.
- **Практическое владение библиотекой модулей Ansible:**
  ...
  - `ansible.builtin.get_url` — безопасная загрузка официальных GPG-ключей подписи пакетов;
  - `ansible.builtin.file` — создание каталогов, управление правами доступа и владельцами;
  - `ansible.builtin.copy` и `ansible.builtin.unarchive` — оптимизированная доставка файлов и распаковка исходных текстов;
  - `ansible.builtin.user` — добавление системных пользователей в служебные группы (включение в группу `docker`);
  - `ansible.builtin.service` / `ansible.builtin.systemd` — запуск, перезапуск и перевод демонов в автозагрузку;
  - `ansible.builtin.lineinfile` — потоковое редактирование конфигурационных файлов ОС и сервисов с использованием регулярных выражений.
- **Оптимизация сетевой передачи файлов:** идентификация проблемы деградации скорости модуля `ansible.builtin.copy` при обработке тысяч мелких файлов исходного кода Java/Maven (посегментный расчет контрольных сумм MD5/SHA256 по SSH) и переход на предварительную архивацию (`tar.gz`) с потоковой распаковкой модулем `ansible.builtin.unarchive` непосредственно на целевом хосте.
- **Архитектурная декомпозиция на Ansible Roles:** трансформация линейного монолитного сценария в структуру повторно используемых ролей:
  - `application`: подготовка среды Docker Engine, сборка образов, запуск 9 микросервисов стека через Docker Compose;
  - `apache`: развертывание и контроль состояния веб-сервера Apache HTTP Server;
  - `postgres`: установка СУБД PostgreSQL, модификация параметров прослушивания внешних интерфейсов `listen_addresses = '*'` в `postgresql.conf`, разрешение клиентских подсетей в `pg_hba.conf`, автоматическое создание тестовой базы `testbd` и наполнение таблицы `Person`.
- **Автоматизация сквозного тестирования:** запуск консольного раннера **Newman** внутри эфемерного Docker-контейнера в режиме сети `--network host` для комплексного прогона интеграционных тестов Postman по API Gateway и Session Service.
...
- **Конфигурирование в нотации HCL (HashiCorp Configuration Language):**
  - Серверный режим (`consul_server.hcl`): директивы `server = true`, `bootstrap_expect = 1` для инициализации кворума Raft, включение встроенного графического веб-интерфейса `ui_config { enabled = true }`, привязка к адаптерам через сетевые макросы шаблонизатора Go `{{ GetInterfaceIP "eth1" }}`;
  - Клиентский режим (`consul_client.hcl`): отключение серверных функций `server = false`, автоматическое присоединение к кластеру по директиве `retry_join = ["192.168.56.10"]`, активация gRPC-интерфейса на порту `8502` для передачи xDS-конфигураций прокси-серверам.
- **Регистрация сервисов и интеграция Service Mesh (Consul Connect):**
  ...
  - Настройка входящих и исходящих зависимостей (`upstreams`) для сервиса `hotel-service` с пробросом виртуального локального порта `local_bind_port = 5432` к удаленной базе данных через mTLS.
- **Оркестрация Sidecar-прокси Envoy:**
  ...
  - Диагностика и устранение версионных несовместимостей: анализ журналов `journalctl`, выявление несовместимости Consul 2.0 CE с версией Envoy 1.39.x, интеграция загрузки бинарного релиза **Envoy v1.38.4** в плейбук Ansible.
- **Декларативное управление PostgreSQL через коллекцию `community.postgresql`:**
  ...
  - Разрешение зависимостей Python: установка драйвера `python3-psycopg2` и пакета `acl` для безопасного переключения контекста выполнения под непривилегированного системного пользователя `postgres` (`become_user: postgres`).
- **Компиляция и оркестрация Java Spring Boot микросервиса:**
  - Автоматизация установки системного стека OpenJDK 21 и Apache Maven;
  - Сборка исполняемого jar-архива средствами Maven Wrapper с параметром `creates` для предотвращения холостых пересборок;
  - Проектирование системного сервиса `hotels.service` с зависимостями `After=consul-envoy.service` и `Requires=consul-envoy.service`, исключающими запуск приложения до готовности сетевого прокси;
  - Изоляция конфигурации базы данных в `/etc/environment` (`POSTGRES_HOST="127.0.0.1"`).
- **Экспериментальное подтверждение динамического Service Discovery:**
  - Имитация сетевой аварии и перемещения узла базы данных: ручная смена IP-адреса хоста `db` на `192.168.56.99`;
  - Фиксация моментального автообновления сервис-каталога Consul через Serf Gossip;
  - Демонстрация безошибочной обработки пользовательских CRUD-запросов клиентом `hotel-service` без перезапуска приложения и без внесения правок в конфигурационные файлы.
...
1. **Инициализация соединения:** Java-приложение `hotel-service` на узле `api` выполняет подключение к СУБД, используя стандартный адрес `127.0.0.1:5432`, считанный из `/etc/environment`.
2. **Перехват локальным прокси:** Локальный процесс Envoy Sidecar на узле `api` слушает порт `5432` и перехватывает исходящее TCP-соединение.
3. **Динамический резолвинг адреса (Control Plane xDS):** Envoy запрашивает актуальный сетевой адрес апстрима `db` у локального агента Consul Client по gRPC-каналу (порт `8502`).
...
5. **Защищенная передача данных:** Локальный Envoy устанавливает зашифрованный взаимно аутентифицированный туннель (mTLS) к удаленному экземпляру Envoy на узле `db`.
...
7. **Поведение при смене IP базы данных:** При смене IP-адреса ноды `db` с `192.168.56.13` на `192.168.56.99` Serf LAN моментально оповещает кластер. Сервер Consul обновляет каталог, а агент Consul Client на ноде `api` через xDS моментально перестраивает маршрут в памяти Envoy. Приложение продолжает непрерывную работу без сбоев (Zero-Downtime).
...
| **Service Discovery & Mesh** | HashiCorp Consul | 2.0.x CE | Распределенный сервис-каталог, Serf LAN Gossip, Raft консенсус, Web UI |
...
| **Стек микросервисов** | Spring Boot / Java | JDK 21 / Maven 3 | Высокопроизводительные REST API сервисы (Hotels, Gateway, Session и др.) |
